    ...
    <table>
        ...
        <tbody>
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                    <td style='width:60px;text-align:center' class="id"><?php echo ($vol["id"]); ?></td>
                    <td style='width:180px;text-align:center' class="name"><?php echo ($vol["title"]); ?> </td>
                    <td class="file"><img src="<?php echo ($vol["thumb"]); ?>"/>
                     <?php if(!empty($vol["picture"])): ?>【<a href="/index.php/Admin/Knowledge/download/id/<?php echo ($vol["id"]); ?>">下载</a>】<?php endif; ?></td>
                    <td style='width:300px;text-align:center' class="description"><?php echo (msubstr($vol["description"],0,8)); ?></td>
                    <td style='width:350px;text-align:center' class="content"><?php echo (msubstr($vol["content"],0,15)); ?> </td>
                    <td style='width:200px;text-align:center' class="author"><?php echo ($vol["author"]); ?> </td>
                    <td style='width:200px;text-align:center' class="addtime"><?php echo (date('Y-m-d H:i:s',$vol["addtime"])); ?> </td>
                    <td style='width:200px;text-align:center' class="operate"> <a href="javascript:;" class="show" data="<?php echo ($vol["id"]); ?>" data-title="<?php echo ($vol["title"]); ?>">查看</a>
