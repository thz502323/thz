    ...
    <table>
        ...
        <tbody>
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
                    <td style='width:100px;text-align:center' class="id"><?php echo ($vol["id"]); ?></td>
                    <td style='width:200px' class="name"><?php echo (msubstr($vol["title"],0,8)); ?></td>
                    <td style='width:300px' class="file"><?php echo ($vol["filename"]); if($vol["hasfile"] == 1): ?><a class='hasfile' href="/index.php/Admin/Doc/download/id/<?php echo ($vol["id"]); ?>">【下载】</a><?php endif; ?></td>
                    <td style='width:100px;text-align:center' class="content"><?php echo ($vol["author"]); ?> </td>
                    <td style='width:200px;text-align:center' class="addtime"><?php echo (date('Y-m-d H:i:s',$vol["addtime"])); ?> </td>
                    <td style='width:100px' class="operate"><a class='show' data='<?php echo ($vol["id"]); ?>' file-name='<?php echo ($vol["title"]); ?>' href="javascript:;">查看</a> |
                    <a href="/index.php/Admin/Doc/edit/id/<?php echo ($vol["id"]); ?>" title="编辑">编辑</a>
                    </td>
